...
def test_5day_quick():
    ...
    try:
        ...
        print(f"生成了 {len(image_set)} 张5天图像")
        ...
        if len(image_set) > 0:
            ...
            # 保存第一张5天图像
            print("\n保存第一张5天图像...")
            # 5天图像尺寸是32x15，使用1:1像素比例显示
            fig_width = 15 * 0.1  # 15像素宽度，每像素0.1英寸
            fig_height = 32 * 0.1  # 32像素高度，每像素0.1英寸
            plt.figure(figsize=(fig_width, fig_height))
            plt.imshow(image.squeeze(), cmap='gray', aspect=1)  # aspect=1 保持1:1像素比例
            plt.title('5天窗口OHLC图像 - 第一张样本')
            plt.xlabel('时间（像素）')
            plt.ylabel('价格/成交量（像素）')
            plt.tight_layout()
            plt.savefig('test_5day_first_image.png', dpi=150, bbox_inches='tight')
            plt.close()
            print("✓ 5天第一张图像已保存为 test_5day_first_image.png")
            ...
            if len(image_set) > 1:
                ...
                # 每个子图使用1:1比例，子图间距适当
                fig_width = 2 * 15 * 0.1 + 1  # 2列，每列15像素宽度，加上间距
                fig_height = 2 * 32 * 0.1 + 1  # 2行，每行32像素高度，加上间距
                ...
                for i in range(num_show):
                    ...
                    axes[i].set_title(f'5天图像 {i+1} - RET5: {r5}')
                ...
                plt.suptitle('5天窗口OHLC图像样本对比', fontsize=16)
                plt.tight_layout()
                plt.savefig('test_5day_multiple_images.png', dpi=150, bbox_inches='tight')
                plt.close()
                print(f"✓ 5天多张图像对比已保存为 test_5day_multiple_images.png")
...
def main():
    ...
    # 运行5天测试
    success = test_5day_quick()
    
    if success:
        print(f"\n✅ 测试成功完成！")
        print(f"   请查看生成的图像文件:")
        print(f"   - test_5day_first_image.png")
        if os.path.exists('test_5day_multiple_images.png'):
            print(f"   - test_5day_multiple_images.png")
    else:
        print(f"\n❌ 测试失败，请检查错误信息")
