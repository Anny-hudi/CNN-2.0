...
class CNN60d(nn.Module):
    ...
    def __init__(self):
        ...
        self.conv1 = nn.Sequential(OrderedDict([
            ('Conv', nn.Conv2d(1, 64, (5, 3), padding=(3, 1), stride=(3, 1), dilation=(3, 1))), # output size: [N, 64, 32, 180]
            ('BN', nn.BatchNorm2d(64, affine=True)),
            ('LeakyReLU', nn.LeakyReLU()),
        ...
        self.conv2 = nn.Sequential(OrderedDict([
            ('Conv', nn.Conv2d(64, 128, (5, 3), padding=(3, 1), stride=(1, 1), dilation=(1, 1))), # output size: [N, 128, 18, 180]
            ('BN', nn.BatchNorm2d(128, affine=True)),
            ('LeakyReLU', nn.LeakyReLU()),
            ('Max-Pool', nn.MaxPool2d((2,1))) # output size: [N, 128, 9, 180]
        ]))
        self.conv2 = self.conv2.apply(self.init_weights)
        
        self.conv3 = nn.Sequential(OrderedDict([
            ('Conv', nn.Conv2d(128, 256, (5, 3), padding=(2, 1), stride=(1, 1), dilation=(1, 1))), # output size: [N, 256, 9, 180]
            ('BN', nn.BatchNorm2d(256, affine=True)),
            ('LeakyReLU', nn.LeakyReLU()),
            ('Max-Pool', nn.MaxPool2d((2,1))) # output size: [N, 256, 4, 180]
        ]))
        self.conv3 = self.conv3.apply(self.init_weights)
        
        self.conv4 = nn.Sequential(OrderedDict([
            ('Conv', nn.Conv2d(256, 512, (5, 3), padding=(2, 1), stride=(1, 1), dilation=(1, 1))), # output size: [N, 512, 4, 180]
        ...
        # 512 * 2 * 180 = 184320, we need 184320
        # 184320 / 512 = 360, so we need 2 * 180 = 360
        # 不需要额外池化，尺寸已经匹配
        self.extra_pool = nn.MaxPool2d((1, 1))  # 不进行额外池化，保持 [N, 512, 2, 180]

        self.DropOut = nn.Dropout(p=0.5)
        self.FC = nn.Linear(184320, 2)  # 512 * 2 * 180 = 184320
        self.init_weights(self.FC)
        self.Softmax = nn.Softmax(dim=1)
    ...
    def forward(self, x): # input: [N, 96, 180]
        ...
        x = self.conv2(x) # output size: [N, 128, 9, 180]
        x = self.conv3(x) # output size: [N, 256, 4, 180]
        x = self.conv4(x) # output size: [N, 512, 2, 180]
        x = self.extra_pool(x) # output size: [N, 512, 2, 180]
        x = self.DropOut(x.view(x.shape[0], -1))
        x = self.FC(x) # output size: [N, 2]
        x = self.Softmax(x)
